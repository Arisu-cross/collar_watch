...
- **经期流量（2026-09-05 起）**：翻译成人话再给 agent（`1 未标 / 2 少量 / 3 中量 /
  4 大量 / 5 无（已结束）`），认不出来的值原样带出去，不硬套。
  这一项取自 `latest.json`，**不受原始样本 48h 清理影响**——它一个月才记几次，
  只看 48h 窗口的话大部分日子都是空的。所以它必然带「多久以前」：
  「中量，just now」和「中量，5 days ago」意思完全不同。
